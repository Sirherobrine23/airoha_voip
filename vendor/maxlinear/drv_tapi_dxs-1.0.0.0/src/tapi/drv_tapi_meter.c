...
static IFX_void_t  ifx_tapi_Meter_OnTimer(Timer_ID Timer, IFX_uintptr_t nArg);
...
{
   TAPI_METER_DATA_t *pTapiMeterData = IFX_NULL;
   ...
   /* Store pointer to data in the channel before we loose it on exit. */
   pChannel->pTapiMeterData = pTapiMeterData;

   /* everything is ok -- this is the regular exit to this function */
   return TAPI_statusOk;
}
                                    ...
                                    IFX_TAPI_METER_START_t const *pMeterStart)
{
   ...
   if (pMeterStart->nPulseDist > UINT_MAX / 1000)
   {
                                     ...
                                     IFX_TAPI_METER_CFG_t const *pMeterConfig)
{
   ...
   if (pMeterConfig == IFX_NULL ||
       pMeterConfig->bMode < IFX_TAPI_METER_MODE_TTX ||
       pMeterConfig->bMode > IFX_TAPI_METER_MODE_REVPOL ||
       pMeterConfig->nPulseLen == 0 ||
       pMeterConfig->nPauseLen == 0)
   {
      RETURN_STATUS (TAPI_statusParam, 0);
   }
                                    ...
                                    IFX_TAPI_METER_BURST_t const *pMeterBurst)
{
   ...
   if (IFX_TAPI_PtrChk (pDrvCtx->ALM.Metering_Start))
   {
      IFX_int32_t retLL = pDrvCtx->ALM.Metering_Start(pChannel->pLLChannel,
                                                      pMeterBurst->nPulses);
