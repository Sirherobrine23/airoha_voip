...
/**
   ...
   \remark
   This file (drv_config_user.lgb.h) is already prepared for systems
   with the LGB chipset - but needs to be adapted to the individual design.
...
/** SPI CS (chip select) set/unset.
 *  On LGM it is controlled by the hardware so define the macro as empty.
*/
#define SPI_CS_SET(nDevNr, high_low) do { /* no operation */ } while(0);
