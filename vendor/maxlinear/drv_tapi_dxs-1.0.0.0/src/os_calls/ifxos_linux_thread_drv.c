...
#include <ifx_types.h>
#include "../include/os_calls/ifxos_rt_if_check.h"
#include "../include/os_calls/ifxos_linux_drv.h"
#include "../include/os_calls/ifxos_debug.h"
#include "../include/os_calls/ifxos_time.h"
#include "../include/os_calls/ifxos_thread.h"
...
\return
   ...
   - IFX_ERROR on error
*/
IFXOS_STATIC int LINUX_KernelThreadStartup(void *data)
{
   ...
   if (pThrCntrl->nPriority)
      LINUX_ThreadPriorityModify(pThrCntrl->nPriority);
               ...
               IFX_ulong_t    nArg2)
{
   ...
   if(pThrCntrl)
   {
      if (IFXOS_THREAD_INIT_VALID(pThrCntrl) == IFX_FALSE)
      {
         ...
         strncpy(pThrCntrl->thrParams.pName, pName, IFXOS_THREAD_NAME_LEN - 1);
         ...
         pThrCntrl->tid = kernel_thread( (IFXOS_KERNEL_THREAD_StartRoutine)LINUX_KernelThreadStartup,
                        (void *)pThrCntrl,
                        IFXOS_DRV_THREAD_OPTIONS);
#else
         pThrCntrl->tid = kthread_run(LINUX_KernelThreadStartup, (void *)pThrCntrl, pName);
#endif
   ...
   if(pThrCntrl)
   {
      if (IFXOS_THREAD_INIT_VALID(pThrCntrl) == IFX_TRUE)
      {
         if (pThrCntrl->thrParams.bRunning == 1)
         {
            ...
            {
               LINUX_MSecSleep(IFXOS_THREAD_DOWN_WAIT_POLL_MS);
