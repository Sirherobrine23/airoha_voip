...
#include <ifx_types.h>
#include "../include/os_calls/ifxos_common.h"
#include "../include/os_calls/ifxos_memory_alloc.h"
#include "../include/os_calls/ifxos_lock.h"
#include "../include/os_calls/ifxos_event.h"
...
/**
   LINUX Kernel - Get the Lock (not interruptible).
      ...
      if (IFXOS_LOCK_INIT_VALID(lockId) == IFX_TRUE)
      {
         switch(timeout_ms)
         {
            ...
            default:
            {
               ...
               if (LINUX_EventInit(&timeoutEvent) != IFX_SUCCESS)
               {
                  return IFX_ERROR;
               }
               ...
               for (timeCnt = 0; ; timeCnt+=10)
               {
                  ...
                  if (LINUX_EventWait (&timeoutEvent, 10, IFX_NULL) != IFX_SUCCESS)
                  {
                     return IFX_ERROR;
                  }
               }
